...
struct FileIdOrd(NonZeroU16);

impl From<FileId> for FileIdOrd {
    fn from(id: FileId) -> Self {
        Self(id.into_raw())
    }
}

impl From<FileIdOrd> for FileId {
    fn from(id: FileIdOrd) -> Self {
        Self::from_raw(id.0)
    }
}
...
type VfsAccessModel<M> = OverlayAccessModel<FileIdOrd, ResolveAccessModel<VfsPathAccessModel<M>>>;
...
impl<M: PathAccessModel + Clone + Sized> Vfs<M> {
    ...
    pub fn is_clean_compile(&self, rev: usize, file_ids: &[FileId]) -> bool {
        ...
        for id in file_ids {
            let Some(entry) = m.entries.get_mut(&FileIdOrd::from(*id)) else {
...
impl<M: PathAccessModel + Sized> RevisingVfs<'_, M> {
    ...
    pub fn reset_shadow(&mut self) {
        ...
        for fid in self.am().file_paths() {
            self.invalidate_file_id(fid.into(), None);
        }
    ...
    pub fn map_shadow_by_id(&mut self, file_id: FileId, snap: FileSnapshot) -> FileResult<()> {
        ...
        self.am().add_file(&file_id.into(), snap, |c| *c);
    ...
    pub fn remove_shadow_by_id(&mut self, file_id: FileId) {
        ...
        self.am().remove_file(&file_id.into());
    }
...
#[derive(Debug, Clone, Default)]
struct EntryMap {
    entries: RedBlackTreeMapSync<FileIdOrd, VfsEntry>,
}
...
impl EntryMap {
    ...
    fn slot<T>(&mut self, path: FileId, f: impl FnOnce(&mut VfsEntry) -> T) -> T {
        if let Some(entry) = self.entries.get_mut(&FileIdOrd::from(path)) {
            f(entry)
        } else {
            let mut entry = VfsEntry::default();
            let res = f(&mut entry);
            self.entries.insert_mut(FileIdOrd::from(path), entry);
            res
        }
    }
