...
impl PathResolution {
    ...
    pub fn join(&self, path: &str) -> FileResult<PathResolution> {
        match self {
            ...
            PathResolution::Rootless(root) => {
                Ok(PathResolution::Rootless(Cow::Owned(root.join(path).map_err(|e| FileError::Other(Some(eco_format!("{e:?}"))))?)))
            }
        }
    }
    ...
    pub fn resolve_to(&self, path: &VirtualPath) -> Option<PathResolution> {
        match self {
            ...
            PathResolution::Rootless(root) => {
                let resolved = path.resolve(root.as_ref().as_rooted_path())?;
                let vpath = VirtualPath::new(resolved.to_str()?).ok()?;
                Some(PathResolution::Rootless(Cow::Owned(vpath)))
            }
        }
    }
}
    ...
    fn path_for_id(&self, file_id: FileId) -> FileResult<PathResolution> {
        ...
        let root = match WorkspaceResolver::resolve(file_id)? {
            ...
            UntitledRooted(..) | Rootless => {
                return Ok(PathResolution::Rootless(Cow::Borrowed(file_id.get().vpath())));
            }
        };
...
impl WorkspaceResolver {
    ...
    pub fn workspace_file(root: Option<&ImmutPath>, path: VirtualPath) -> FileId {
        ...
        let root = match workspace {
            Some(id) => VirtualRoot::Package(id.package()),
    ...
    pub fn rooted_untitled(root: Option<&ImmutPath>, path: VirtualPath) -> FileId {
        let workspace = root.map(Self::workspace_id);
        let root = match workspace {
            Some(id) => VirtualRoot::Package(id.untitled_root()),
            None => VirtualRoot::Project,
        };
        RootedPath::new(root, path).intern()
    }
