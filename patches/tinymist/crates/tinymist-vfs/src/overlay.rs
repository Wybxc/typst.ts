...
use crate::{AccessModel, Bytes, FileId, FileIdOrd, FileSnapshot, PathAccessModel};
...
impl<M: AccessModel> AccessModel for OverlayAccessModel<FileIdOrd, M> {
    fn reset(&mut self) {
        self.inner.reset();
    }

    fn content(&self, src: FileId) -> (Option<ImmutPath>, FileResult<Bytes>) {
        if let Some(content) = self.files.get(&FileIdOrd::from(src)) {
