...
impl<F: CompilerFeat> CompilerUniverse<F> {
    ...
    pub fn id_for_path(&self, path: &Path) -> Option<FileId> {
        ...
        let vpath = VirtualPath::new(stripped.to_str()?).ok()?;
...
impl<F: CompilerFeat> World for CompilerWorld<F> {
    ...
    fn today(&self, offset: Option<typst::foundations::Duration>) -> Option<Datetime> {
        ...
        let naive = match offset {
            ...
            Some(o) => now.naive_utc() + Duration::try_hours(o.hours() as i64)?,
        };
    ...
    fn today(&self, offset: Option<typst::foundations::Duration>) -> Option<Datetime> {
        ...
        let now = offset
            .and_then(|offset| {
                let dur = Duration::from_secs(offset.as_secs().try_into().ok()?)
                    .try_into()
                    .ok()?;
                now.checked_add(dur)
            })
