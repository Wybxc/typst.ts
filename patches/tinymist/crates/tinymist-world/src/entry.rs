...
pub static DETACHED_ENTRY: LazyLock<FileId> =
    LazyLock::new(|| RootedPath::new(VirtualRoot::Project, VirtualPath::new("/__detached.typ").expect("/__detached.typ")).intern());

/// The memory main entry.
pub static MEMORY_MAIN_ENTRY: LazyLock<FileId> =
    LazyLock::new(|| RootedPath::new(VirtualRoot::Project, VirtualPath::new("/__main__.typ").expect("/__main__.typ")).intern());
...
impl EntryState {
    ...
    pub fn new_rooted_by_parent(entry: ImmutPath) -> Option<Self> {
        ...
        let main =
            WorkspaceResolver::workspace_file(root.as_ref(), VirtualPath::new(entry.file_name()?.to_str()?).ok()?);
    ...
    pub fn select_in_workspace(&self, path: &Path) -> EntryState {
        let vpath = VirtualPath::new(path.to_str().unwrap()).unwrap();
        let id = WorkspaceResolver::workspace_file(self.root.as_ref(), vpath);
        ...
        Ok(match self.workspace_root() {
            Some(root) => match path.strip_prefix(&root) {
                Ok(path) => Some(EntryState::new_rooted(
                    root.clone(),
                    Some(VirtualPath::new(path.to_str().unwrap()).unwrap()),
                )),
                Err(err) => {
                    return Err(
    ...
    fn try_from(value: EntryOpts) -> Result<Self, Self::Error> {
        match value {
            EntryOpts::Workspace { root, main: entry } => Ok(EntryState::new_rooted(
                root.as_path().into(),
                entry.and_then(|p| p.to_str().and_then(|s| VirtualPath::new(s).ok())),
            )),
